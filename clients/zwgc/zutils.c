...
Code_t set_exposure(zrealm, exposure)
     char *zrealm;
     char *exposure;
{
    ...
    if (zrealm && strcmp(zrealm, "*") == 0) {
	if (retval = ZGetRealmCount(&cnt))
		return(retval);

	for (i=0; i<cnt; i++) {
	    if (retval = ZGetRealmName(i, &zrealm))
		return(retval);

	    sprintf(zvar, "exposure-%s", zrealm);
				
	    if (realm_exp_level = ZGetVariable(zvar)) {
		if (strcmp(realm_exp_level, EXPOSE_NETVIS) == 0)
		    realm_exp_level = EXPOSE_REALMVIS;
		else
		    realm_exp_level = ZParseExposureLevel(realm_exp_level);
	    } else if (realm_exp_level = exp_level) {
		if (strcmp(realm_exp_level, EXPOSE_NETVIS) == 0)
		    realm_exp_level = EXPOSE_REALMVIS;

		if (i > 0) {
		    if (strcmp(realm_exp_level, EXPOSE_REALMVIS) == 0)
			realm_exp_level = EXPOSE_OPSTAFF;
		    else if (strcmp(realm_exp_level, EXPOSE_REALMANN) == 0)
			realm_exp_level = EXPOSE_OPSTAFF;
		} else {
		    realm_exp_level = ZParseExposureLevel(realm_exp_level);
		}
	    } else {
		realm_exp_level = EXPOSE_NONE;
	    }
...
Code_t load_sub_file(type, file, realm)
	int type;
	char *file;
	char *realm;
{
	...
	for (;;lineno++) {
	    ...
	    *comma2 = '\0';
		...
		if ((retval = (type == SUB)?
		     ZSubscribeTo(realm, subs2,ind,(u_short)wgport):
		     ZUnsubscribeTo(realm, subs2,ind,(u_short)wgport)) !=
		    ZERR_NONE) {
		    goto cleanup;
		}
		for (i=0;i<ind;i++) {
		...
		if ((retval = ZUnsubscribeTo(realm, subs2,unind,(u_short)wgport)) != ZERR_NONE) {
		    goto cleanup;
		}
		for (i=0;i<unind;i++) {
	...
	fix_macros(subs,subs2,ind);
	if ((retval = (type == SUB)?ZSubscribeTo(realm, subs2,ind,(u_short)wgport):
	     ZUnsubscribeTo(realm, subs2,ind,(u_short)wgport)) != ZERR_NONE) {
	    goto cleanup;
	}
	if (unind) {
	    fix_macros(unsubs,subs2,unind);
	    if ((retval =
		 ZUnsubscribeTo(realm, subs2,unind,(u_short)wgport)) != ZERR_NONE) {
		goto cleanup;
	    }
	}
    }
