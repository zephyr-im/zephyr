...
void zwgc_shutdown()
{
	...
	TRAP(ZRetrieveSubscriptions(realm, 0,
	...
	TRAP(ZCancelSubscriptions(realm, 0), "while canceling subscriptions");
    }
...
void zwgc_startup()
{
	...
	for (i=0; i<cnt; i++) {
		...
		TRAP(ZSubscribeToSansDefaults(realm, saved_subscriptions[i],
					      number_of_saved_subscriptions[i],
					      0),
