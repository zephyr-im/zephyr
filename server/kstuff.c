...
#define KRB5AUTHENT &authenticator
    ...
    /* HOLDING: authbuf, authctx, keytabid */
	...
	syslog(LOG_DEBUG, "k5 auth failed from %s (%8x:%8x:%8x): %s", realm,
	       notice->z_uid.zuid_addr.s_addr, notice->z_uid.tv.tv_sec, notice->z_uid.tv.tv_usec,
	       error_message(result));
      else
        syslog(LOG_WARNING,"k5 auth failed from %s (%8x:%8x:%8x): %s", realm,
	       notice->z_uid.zuid_addr.s_addr, notice->z_uid.tv.tv_sec, notice->z_uid.tv.tv_usec,
	       error_message(result));
      free(authbuf);
