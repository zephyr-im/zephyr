    ...
    int ack_to_sender;
{
    ...
    if (!ticket_lookup(realm->name))
	if ((retval = ticket_retrieve(realm)) != ZERR_NONE) {
	    syslog(LOG_WARNING, "rlm_handoff %s failed: %s", realm->name,
		   error_message(retval));
	    realm_sendit(notice, who, auth, realm, ack_to_sender);
	    return;
	}
    ...
    int ack_to_sender;
{
	...
	while (offset < notice->z_message_len || !notice->z_message_len) {
	    ...
	    if (offset > 0) {
	        Z_gettimeofday(&now, (struct timezone *)NULL);
	        partnotice.z_uid.tv.tv_sec = now.tv_sec;
	        partnotice.z_uid.tv.tv_usec = now.tv_usec;
