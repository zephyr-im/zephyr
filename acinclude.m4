...
dnl
dnl	ATHENA_RESOLV_CHECK
dnl		Checks to see if the resolver library is present and
dnl		provides "res_send" or "__res_send" (the latter is used
dnl		on Linux).
...
AC_DEFUN(ATHENA_RESOLV_CHECK,
r=false
AC_CHECK_FUNC(res_send, :,
	AC_CHECK_LIB(resolv, res_send, r=true,
			AC_CHECK_LIB(resolv, __res_send, r=true)))
if $r ; then
...
AC_REQUIRE([ATHENA_RESOLV_CHECK])
...
AC_CHECK_LIB(krb4, krb_rd_req,
	     ...
	     [AC_CHECK_LIB(krb, krb_rd_req,
			   [KRB4_LIBS="-lkrb -ldes -lroken -lcom_err"],
			   [AC_MSG_ERROR(Kerberos 4 libraries not found)],
			   -ldes -lroken -lcom_err)],
